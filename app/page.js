...
// Real data only — pulled at request time from legitimate public APIs.
// No fabricated content. Each source route normalizes to the same shape;
// fields a source genuinely can't provide (e.g. TikTok view counts) stay null
// and the UI shows "—" rather than inventing a number.
async function fetchRealTrends(source, param) {
  const routes = {
    youtube: `/api/trends/youtube?q=${encodeURIComponent(param)}`,
    reddit: `/api/trends/reddit?sub=${encodeURIComponent(param)}`,
    tiktok: `/api/trends/tiktok?url=${encodeURIComponent(param)}`,
  };
  const res = await fetch(routes[source]);
  const data = await res.json();
  if (!res.ok) {
    throw new Error(data?.error?.error?.message || data?.error || "Fetch failed");
  }
  return (data.items || []).map((it) => ({
    ...it,
    engagement: (it.views && (it.likes != null || it.comments != null))
      ? (((it.likes || 0) + (it.comments || 0) + (it.shares || 0)) / it.views) * 100
      : null,
  }));
}
...
export default function App() {
  ...
  useEffect(() => {
    // Populate the dashboard with one real default pull on first load.
    (async () => {
      setViralLoading(true);
      try {
        const results = await fetchRealTrends("reddit", "personalfinance");
        setViral(results);
      } catch (e) {
        setViralError(e.message);
      } finally {
...
function Dashboard({ viral, viralLoading, library, offers, setTab, openWorkflow }) {
  const metrics = [
  ...
  const rank = (v) => (v.views || 0) + (v.likes || 0) * 10 + (v.comments || 0) * 5;
  const topViral = [...viral].sort((a, b) => rank(b) - rank(a)).slice(0, 3);
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 16 }}>
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {viralLoading && <EmptyState icon={Radar} title="Pulling real trend data..." sub="Fetching from Reddit's public feed." />}
            {!viralLoading && topViral.length === 0 && <EmptyState icon={Radar} title="No data yet" sub="Head to Viral Radar to pull real data from YouTube Shorts, Reddit, or a TikTok URL." />}
            {!viralLoading && topViral.map((v) => (
    ...
    <Card hoverable style={{ display: "flex", gap: 14, padding: 14 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 4 }}>
          ...
          {v.url && (
            <a href={v.url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} style={{ fontSize: 11, color: COLORS.amber, textDecoration: "none" }}>View source ↗</a>
          )}
        </div>
        <div style={{ fontWeight: 600, fontSize: compact ? 13 : 14, marginBottom: 6, lineHeight: 1.35 }}>{v.hookText}</div>
        {!compact && (v.type || v.trigger) && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
            {v.type && <Badge>{v.type}</Badge>}
            {v.trigger && <Badge tone="violet">{v.trigger}</Badge>}
            {!v.type && !v.trigger && <Badge>Not yet classified — click Analyze</Badge>}
          </div>
        )}
        <div style={{ display: "flex", gap: 14, fontSize: 11.5, color: COLORS.dim, fontFamily: "monospace", marginBottom: compact ? 0 : 10, flexWrap: "wrap" }}>
          ...
          <span>{v.engagement != null ? v.engagement.toFixed(1) + "% eng." : "—"}</span>
        </div>
        {v.statsUnavailable && (
          <div style={{ fontSize: 11, color: COLORS.dim, marginBottom: 8, fontStyle: "italic" }}>TikTok doesn't expose view/engagement stats publicly — title and creator are real, metrics aren't available.</div>
        )}
        {!compact && (
...
function ViralRadar({ viral, setViral, viralLoading, setViralLoading, viralError, setViralError, savedViralIds, setSavedViralIds, openWorkflow, showToast }) {
  const [sortBy, setSortBy] = useState("viral");
  const [minViews, setMinViews] = useState(0);
  const [search, setSearch] = useState("");
  const [source, setSource] = useState("youtube");
  const [queryInput, setQueryInput] = useState("budgeting tips");
  const [subInput, setSubInput] = useState("personalfinance");
  const [tiktokUrl, setTiktokUrl] = useState("");
  const [appendMode, setAppendMode] = useState(false);

  async function runFetch() {
    const param = source === "youtube" ? queryInput : source === "reddit" ? subInput : tiktokUrl;
    if (!param.trim()) { showToast("Enter a search term / subreddit / URL first", true); return; }
    setViralLoading(true);
    setViralError(null);
    try {
      const results = await fetchRealTrends(source, param);
      if (results.length === 0) showToast("No results — try a different query", true);
      setViral((prev) => appendMode ? [...results, ...prev.filter((p) => !results.find((r) => r.id === p.id))] : results);
    } catch (e) {
      setViralError(e.message);
  ...
  return (
    <div style={{ animation: "fadeIn 0.2s ease" }}>
      <div style={{ position: "relative", borderRadius: 14, overflow: "hidden", padding: 22, marginBottom: 20, background: `radial-gradient(circle at 20% 20%, rgba(245,166,35,0.12), transparent 55%), radial-gradient(circle at 80% 80%, rgba(139,124,246,0.12), transparent 55%)`, border: `1px solid ${COLORS.border}` }}>
        ...
        <p style={{ color: COLORS.dim, fontSize: 13, margin: 0 }}>Real data only — pulled live from YouTube, Reddit, or a TikTok URL you paste in. No fabricated stats.</p>
      </div>

      <Card style={{ marginBottom: 18 }}>
        <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
          <Pill active={source === "youtube"} onClick={() => setSource("youtube")}>YouTube Shorts</Pill>
          <Pill active={source === "reddit"} onClick={() => setSource("reddit")}>Reddit</Pill>
          <Pill active={source === "tiktok"} onClick={() => setSource("tiktok")}>Paste TikTok URL</Pill>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          {source === "youtube" && (
            <Input placeholder="Search term, e.g. budgeting tips" value={queryInput} onChange={(e) => setQueryInput(e.target.value)} style={{ flex: 1, minWidth: 220 }} />
          )}
          {source === "reddit" && (
            <Input placeholder="Subreddit, e.g. personalfinance" value={subInput} onChange={(e) => setSubInput(e.target.value)} style={{ flex: 1, minWidth: 220 }} />
          )}
          {source === "tiktok" && (
            <Input placeholder="Paste a full TikTok video URL" value={tiktokUrl} onChange={(e) => setTiktokUrl(e.target.value)} style={{ flex: 1, minWidth: 220 }} />
          )}
          <Btn variant="primary" icon={viralLoading ? Loader2 : Search} onClick={runFetch} disabled={viralLoading}>{viralLoading ? "Fetching..." : "Fetch Real Data"}</Btn>
        </div>
        {source === "tiktok" && (
          <div style={{ fontSize: 11.5, color: COLORS.dim, marginTop: 8 }}>TikTok's public oEmbed only returns title/author/thumbnail — no view or like counts are available without an approved TikTok API partnership. We show what's real and leave the rest blank.</div>
        )}
        {source === "youtube" && (
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
        <div style={{ flex: 1, minWidth: 200, position: "relative" }}>
          ...
          <Input placeholder="Filter loaded results..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ paddingLeft: 30 }} />
        </div>
        <Select value={sortBy} onChange={setSortBy} options={[
          { value: "viral", label: "Sort: Most Views" },
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {viralLoading && <EmptyState icon={Radar} title="Fetching real data..." sub="Calling the live API — one moment." />}
        {!viralLoading && list.map((v) => (
          <ViralCard key={v.id} v={v} openWorkflow={openWorkflow} saved={savedViralIds.includes(v.id)} onSave={() => toggleSave(v.id)} />
        ))}
        {!viralLoading && list.length === 0 && <EmptyState icon={Radar} title="No results yet" sub="Choose a source above and fetch real data." />}
      </div>
    </div>
  );
}
  ...
  const runAnalysis = useCallback(async () => {
    ...
    try {
      const result = await callClaude(
        `You are a viral content intelligence analyst for a CPA/affiliate marketing tool. You analyze WHY content resonated/went viral — never suggest copying it. The content type and psychological trigger may not be pre-labeled; classify them yourself from the content. Respond with ONLY valid JSON, no markdown fences, matching this exact shape:
{"contentType":"string - one of Storytime, List, Review, POV, Tutorial, Reaction, Confession, Experiment, Comparison, Discovery, Insider, Challenge, Product Showcase","trigger":"string - one of Curiosity, Rage, FOMO, Surprise, Social Proof, Status, Relatability, Controversy, Mystery, Aspiration, Problem/Solution","hook":"string - what happens in first 1-3 seconds / first line","curiosityGap":"string","emotionalTrigger":"string","patternInterrupt":"string","audience":"string","retentionMechanism":"string","payoff":"string","commentTrigger":"string","shareTrigger":"string","replicability":"string - can this be adapted to other niches, explain briefly","scores":{"hook":0-100,"curiosity":0-100,"retention":0-100,"engagement":0-100,"replicability":0-100,"cpaAdaptability":0-100}}`,
        `Analyze this real post:\nTopic/title: ${item.topic}\nOpening line / title: "${item.hookText}"\n${item.description ? `Description: ${item.description}\n` : ""}Platform: ${item.platform}\nViews: ${item.views ?? "not available"}, Likes: ${item.likes ?? "not available"}, Comments: ${item.comments ?? "not available"}${item.engagement != null ? `, Engagement rate: ${item.engagement.toFixed(1)}%` : ""}`
      );
      setAnalysis(result);
    } catch (e) {
...
{"sourceContent":"string - what the original creator actually made","underlyingAngle":"string - the transferable concept","contentStructure":"string - how it was presented","newExecutions":[{"category":"string","angle":"string - the new angle title","description":"string - one sentence"}]} — newExecutions must have exactly 10 items.`,
        `Source viral content:\nTopic: ${item.topic}\nHook/title: "${item.hookText}"\nContent type: ${analysis?.contentType || "unclassified"}\nTrigger: ${analysis?.trigger || "unclassified"}`
      );
      setAngles(result);
...
{"hook":"string","visual":"string - what appears on screen","dialogue":"string - what the creator says, full script","bRoll":"string - suggested footage","textOverlay":"string - on-screen text","retentionBeat":"string - where the next curiosity point is introduced","reveal":"string - what gets revealed","cta":"string - natural CTA line","duration":"15s | 30s | 45s | 60s"}`,
  ...
  function saveEverything() {
    addToLibrary({
      ...
      tags: [chosenAngle.category, analysis?.trigger || item.platform],
    });
    onClose();
  }
          ...
          {!loading && step === "analyze" && analysis && (
            <div>
              ...
              <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
                {analysis.contentType && <Badge>{analysis.contentType}</Badge>}
                {analysis.trigger && <Badge tone="violet">{analysis.trigger}</Badge>}
              </div>
